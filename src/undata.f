...
C
C
C     IF RM_Text ITEM -- LEN1 IS NUMBER OF CHARACTERS
C
  280          IF (ATTYPE .NE. KZTEXT) GO TO 300
C
C  RM_Text PROCESSING SECTION
C
               CALL MSG('R','''','+')
               START = 1
...
C
C  CHECK TO SEE IF THE RM_Text STRING CAN FIT ON THE LINE
C
...
C
               ...
               ENDIF
C
C  DONE WITH RM_Text ITEM -- ADD TRAILING QUOTE
C
  298          CONTINUE
               IF (DONE .EQ. 1) STAT = 1
               CALL MSG('R',''' ','+')

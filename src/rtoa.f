      SUBROUTINE RTOA_X(STRING,SC,FMT,RNUM,IERR)
         ...
         USE RM_Text, only: FILCH, ABLANK, ITOA, RITOA, PLSIGN, MNSIGN
         USE RM_Text, only: DECIM, ASSTAR, UECH, U0

         INCLUDE 'syspar.inc'
C
C     CONVERT A REAL (RNUM) TO ASCII-RM_Text (STRING)
C     IF THE REAL WILL NOT FIT, STRING IS
C     BLANKED OUT AND IERR IS RETURNED NON-ZERO.
C
C     STRING....REPOSITORY FOR RM_Text OF RNUM
